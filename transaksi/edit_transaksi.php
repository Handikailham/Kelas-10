...
$sql = "SELECT tb_transaksi.id_transaksi, tb_pelanggan.id_pelanggan, tb_pelanggan.nama_pelanggan, tb_buku.id_buku, tb_buku.nama_buku, tb_transaksi.jumlah, tb_transaksi.harga_saat_ini, tb_transaksi.total_pembayaran FROM tb_pelanggan INNER JOIN tb_transaksi ON tb_pelanggan.id_pelanggan = tb_transaksi.id_pelanggan INNER JOIN tb_buku ON tb_buku.id_buku = tb_transaksi.id_buku WHERE tb_transaksi.id_transaksi = '$id_transaksi'";
...
<!DOCTYPE html>
    ...
    <body>
        <form action="update_transaksi.php" method="post">
            ...
            <?php
            foreach($options_pelanggan as $option){
                $selected = $option['id_pelanggan']==$row['id_pelanggan'] ? 'selected' : '';
            ?>
                <option value="<?=$option['id_pelanggan']?>" <?= $selected?>><?=$option['nama_pelanggan']?>
                </option>
                <?php } ?>
            </select></br>
            ...
            <?php
            foreach($options_buku as $option){
